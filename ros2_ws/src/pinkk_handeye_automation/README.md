...
유지하면서 local X/Y/Z 회전을 조합한 후보 관측 자세 30개를 자동으로 생성합니다.
...
복귀합니다. 후보 30개를 모두 촬영하는 것이 아니라 유효 샘플 15개를 확보하면
즉시 종료합니다. 모든 후보를 시도해도 15개를 못 모은 경우에는 최소 12개
이상일 때만 계산 및 저장을 수행합니다.
...
후보 자세의 IK가 충분히 성공하면 실제 자동 수집을 실행합니다.

```bash
ros2 launch pinkk_handeye_automation auto_calibrate.launch.py execute:=true
```

목표 및 최소 샘플 수는 필요할 때 launch 인자로 변경할 수 있습니다.
