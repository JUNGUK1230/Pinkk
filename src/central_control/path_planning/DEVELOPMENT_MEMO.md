...
## 2026-08-27: 웹 버튼 기반 단계별 경로 전환

- `entry`는 START→P5~P8, `charge`는 P5~P8→C2/C1 경로만 선택한다.
- 관리자웹의 기존 일시정지 버튼을 `주차칸으로 이동`(`park`)으로 바꾸고,
  C1/C2에 있는 선택 차량만 P1~P4 경로 단계로 전환한다.
- SPACE 키는 화면을 직접 조작할 때 사용할 수 있는 fallback으로 유지한다.

## 2026-07-27: Space 충전 완료 이벤트와 C1/C2 → P1~P4 전환
...
- 해당 `track_id`만 충전 완료 상태로 메모리에 표시하고, 화면상 차량이 아직
  C1/C2에 보이더라도 기존 충전칸 재계획 대신 `charge_to_exit` 정책의 P1~P4
  빈 자리로 Hybrid A*를 다시 생성한다.
- P1~P4는 설정된 출구 기준 거리 순서로 고르며, 차량이 선택 칸에 실제 도착하면
