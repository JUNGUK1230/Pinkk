...
`park(2)`, `replan(3)`, `exit(4)`, `charge(5)`, `entry(6)` 순서입니다.
...
`입차`는 START에서 P5~P8, `충전`은 P5~P8에서 C2 우선·C1 대체,
`주차칸으로 이동`은 C1/C2에서 P1~P4 경로를 요청합니다.
...
경로 재생성, 주차칸으로 이동 또는 긴급 정지 버튼을 실제로 누른 경우에만 해당
