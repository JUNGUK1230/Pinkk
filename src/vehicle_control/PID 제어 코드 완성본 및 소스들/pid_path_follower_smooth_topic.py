...
class WaypointPIDController(Node):
    ...
    WP1→WP7:
      일반 코스 구간. 각 웨이포인트에서 멈추거나 제자리 회전하지 않고
      전진 속도와 회전 속도를 동시에 출력해 연속 곡선주행한다.

    WP7→WP8:
      기존 주차 제어와 동일하게 WP7에서 주차 방향으로 정렬한 뒤
      직선 전진하여 주차한다.
    ...
    def __init__(self) -> None:
        ...
        # 중앙제어 경로 토픽 입력
        self.declare_parameter('planned_path_topic', '/pinkk/planned_path')
        self.declare_parameter('use_planned_path', True)
        self.declare_parameter('planned_path_min_spacing', 0.025)
        self.declare_parameter('planned_path_goal_tolerance', 0.020)
        self.declare_parameter('planned_path_lookahead', 0.060)
        ...
        self.declare_parameter('smooth_end_waypoint_number', 7)
        ...
        self.use_planned_path = bool(
            self.get_parameter('use_planned_path').value
        )
        self.planned_path_min_spacing = float(
            self.get_parameter('planned_path_min_spacing').value
        )
        self.planned_path_goal_tolerance = float(
            self.get_parameter('planned_path_goal_tolerance').value
        )
        self.planned_path_lookahead = float(
            self.get_parameter('planned_path_lookahead').value
        )
        ...
        # 중앙제어 토픽 경로 상태
        self.pending_planned_path: Optional[Path] = None
        self.external_path_active = False
        self.external_waypoints: List[Tuple[float, float, float, bool]] = []
        self.external_target_index = 0
        self.external_previous_yaw_error = 0.0
        self.external_error_initialized = False
        ...
        self.get_logger().info(
            '10개 WP | WP7까지 연속 곡선주행 + 기존 주차 controller started'
        )
        if self.use_planned_path:
            self.get_logger().info(
                f'중앙제어 경로 대기: {self.planned_path_topic} (nav_msgs/Path)'
            )
        else:
            self.get_logger().info(
    ...
    def odom_callback(self, msg: Odometry) -> None:
        ...
        if self.pending_planned_path is not None:
            pending = self.pending_planned_path
            self.pending_planned_path = None
            self.activate_planned_path(pending)

    def planned_path_callback(self, msg: Path) -> None:
        if not self.use_planned_path:
            return

        if len(msg.poses) < 2:
            self.get_logger().warning(
                '수신한 planned_path에 pose가 2개 미만이라 무시합니다.'
            )
            return

        if not self.odom_received:
            self.pending_planned_path = msg
            self.get_logger().info(
                'planned_path 수신 완료, odom 수신 후 경로를 시작합니다.'
            )
            return

        self.activate_planned_path(msg)

    def activate_planned_path(self, msg: Path) -> None:
        first = msg.poses[0].pose
        first_yaw = quaternion_to_yaw(
            float(first.orientation.x),
            float(first.orientation.y),
            float(first.orientation.z),
            float(first.orientation.w),
        )

        rotation = normalize_angle(self.current_yaw - first_yaw)
        cos_r = math.cos(rotation)
        sin_r = math.sin(rotation)
        origin_x = float(first.position.x)
        origin_y = float(first.position.y)

        transformed: List[Tuple[float, float, float]] = []
        for stamped_pose in msg.poses:
            pose = stamped_pose.pose
            rel_x = float(pose.position.x) - origin_x
            rel_y = float(pose.position.y) - origin_y

            odom_x = self.current_x + cos_r * rel_x - sin_r * rel_y
            odom_y = self.current_y + sin_r * rel_x + cos_r * rel_y

            path_yaw = quaternion_to_yaw(
                float(pose.orientation.x),
                float(pose.orientation.y),
                float(pose.orientation.z),
                float(pose.orientation.w),
            )
            odom_yaw = normalize_angle(path_yaw + rotation)
            transformed.append((odom_x, odom_y, odom_yaw))

        sampled: List[Tuple[float, float, float]] = [transformed[0]]
        for point in transformed[1:-1]:
            last = sampled[-1]
            if math.hypot(point[0] - last[0], point[1] - last[1]) >= (
                self.planned_path_min_spacing
            ):
                sampled.append(point)
        sampled.append(transformed[-1])

        external: List[Tuple[float, float, float, bool]] = []
        for index, point in enumerate(sampled):
            if index + 1 < len(sampled):
                next_point = sampled[index + 1]
                move_x = next_point[0] - point[0]
                move_y = next_point[1] - point[1]
            else:
                previous = sampled[index - 1]
                move_x = point[0] - previous[0]
                move_y = point[1] - previous[1]

            heading_x = math.cos(point[2])
            heading_y = math.sin(point[2])
            is_reverse = (move_x * heading_x + move_y * heading_y) < 0.0
            external.append((point[0], point[1], point[2], is_reverse))

        # 첫 pose는 현재 위치 기준점이므로 두 번째 pose부터 추종한다.
        self.external_waypoints = external
        self.external_target_index = 1
        self.external_path_active = True
        ...
        reverse_count = sum(1 for point in external if point[3])
        self.get_logger().info(
            f'planned_path 적용 완료: 원본 {len(msg.poses)}개 → '
            f'추종점 {len(external)}개, 후진점 {reverse_count}개, '
            f'frame={msg.header.frame_id or "(empty)"}'
        )

    def control_external_path(self, dt: float) -> None:
        if self.external_target_index >= len(self.external_waypoints):
            self.stop_robot()
            self.external_path_active = False
            self.state = ControlState.FINISHED
            self.get_logger().info('planned_path 주행 완료')
            return

        target = self.external_waypoints[self.external_target_index]
        target_x, target_y, target_yaw, is_reverse = target

        dx = target_x - self.current_x
        dy = target_y - self.current_y
        distance = math.hypot(dx, dy)

        is_final = self.external_target_index == len(self.external_waypoints) - 1
        tolerance = (
            self.planned_path_goal_tolerance
            if is_final
            else max(self.planned_path_min_spacing * 0.8, 0.012)
        )
        ...
        # 가까운 경로점 여러 개를 건너뛰어 lookahead 지점을 선택한다.
        lookahead_index = self.external_target_index
        accumulated = 0.0
        previous_x = self.current_x
        previous_y = self.current_y
        while lookahead_index + 1 < len(self.external_waypoints):
            candidate = self.external_waypoints[lookahead_index]
            segment = math.hypot(
                candidate[0] - previous_x,
                candidate[1] - previous_y,
            )
            accumulated += segment
            if accumulated >= self.planned_path_lookahead:
                break
            previous_x, previous_y = candidate[0], candidate[1]
            lookahead_index += 1

        aim = self.external_waypoints[lookahead_index]
        aim_yaw = math.atan2(
            aim[1] - self.current_y,
            aim[0] - self.current_x,
        )
        if is_reverse:
            aim_yaw = normalize_angle(aim_yaw + math.pi)

        # 마지막 구간에서는 Path pose의 yaw를 섞어 최종 자세까지 맞춘다.
        if is_final and distance < max(self.planned_path_lookahead * 2.0, 0.10):
            ratio = 1.0 - clamp(
                distance / max(self.planned_path_lookahead * 2.0, 0.10),
                0.0,
                1.0,
            )
            path_error = normalize_angle(target_yaw - self.current_yaw)
            aim_error = normalize_angle(aim_yaw - self.current_yaw)
            yaw_error = normalize_angle(
                (1.0 - ratio) * aim_error + ratio * path_error
            )
        else:
            yaw_error = normalize_angle(aim_yaw - self.current_yaw)

        if self.external_error_initialized:
            yaw_derivative = normalize_angle(
                yaw_error - self.external_previous_yaw_error
            ) / max(dt, 0.001)
        else:
            yaw_derivative = 0.0
            self.external_error_initialized = True
        self.external_previous_yaw_error = yaw_error
        ...
        heading_factor = clamp(
            1.0 - abs(yaw_error) / math.radians(75.0),
            0.25,
            1.0,
        )
        ...
        linear_magnitude = clamp(
            self.smooth_max_linear_speed * heading_factor * distance_factor,
            self.smooth_min_linear_speed,
            self.smooth_max_linear_speed,
        )

        self.publish_velocity(
            linear_x=-linear_magnitude if is_reverse else linear_magnitude,
    ...
    def control_loop(self) -> None:
        ...
        if self.use_planned_path:
            if self.external_path_active:
                self.control_external_path(dt)
            else:
                self.stop_robot()
            return
